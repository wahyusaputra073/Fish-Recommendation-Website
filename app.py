...
def index():
   ...
   # Filter by search query
    ...
    if search_query:
        outputkab = ikan[
            ikan['Jenis Ikan'].str.contains(search_query, case=False) |
            ikan['Kabupaten/Kota'].str.contains(search_query, case=False)
        ]
    elif kota_query:
        outputkab = ikan[ikan['Kabupaten/Kota'].str.contains(kota_query, case=False)]
    else:
        outputkab = ikan.copy()

    # Take one Region
    outputkab = outputkab[['Kabupaten/Kota', 'Jenis Ikan', 'Status']].copy()
